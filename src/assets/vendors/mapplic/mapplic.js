  ...
  var Mapplic = function(element) {
      ...
      this.init = function() {
        ...
        if (self.o.search) {
          ...
          self.clear = $('<button></button>')
            ...
            .appendTo(form);
          var input = $('<input>')
            .attr({ type: 'text', spellcheck: 'false', placeholder: 'Search...' })
        ...
        .resize(function() {
          ...
          if ($(window).width() < 668) {
            if (self.el.hasClass('mapplic-fullscreen')) self.container.height($(window).height());
              ...
              var height = Math.min(
                Math.max(
                  (self.container.width() * self.contentHeight) / self.contentWidth,
                  ($(window).height() * 2) / 3
                ),
                $(window).height() - 66
              );
    ...
    var addControls = function() {
      ...
      hammer.on('pinch', function(e) {
        ...
        self.x = normalizeX(
          self.x - (e.center.x - self.container.offset().left - self.x) * (self.scale / oldscale - 1)
        );
    ...
    var latlngToPos = function(lat, lng) {
      ...
      return {
        ...
        y:
          (self.data.mapheight - ((mapWidth / 2) * Math.log((1 + Math.sin(lat)) / (1 - Math.sin(lat))) - mapOffsetY)) /
          self.data.mapheight
      };
    };
